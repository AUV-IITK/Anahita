...
MarkerDropper::MarkerDropper() : it(nh) {
    ...
    this->bottom_low_v_blue = 0;
    this->bottom_high_h_blue = 255;
    this->bottom_high_s_blue = 29;
	...
	while (ros::ok())
	{
		...
		if (!image_bottom.empty())
		{
		    ...
		    if ((bottom_high_h_ > bottom_low_h_ && bottom_high_s_ > bottom_low_s_ && bottom_high_v_ > bottom_low_v_) && (bottom_high_h_blue > bottom_low_h_blue && bottom_high_s_blue > bottom_low_s_blue && bottom_high_v_blue > bottom_low_v_blue))
			{
				...
				if (contours.size() != 0)
		     	{
					...
					ROS_INFO("Area of contour=%f", contourArea(contours[0]));
					ROS_INFO("Marker Center Location (x, y, z) = (%f, %f, %f)", x_coordinate.data, y_coordinate.data, z_coordinate.data);
				...
				ROS_INFO("Size of blue contour largest=%f", cv::contourArea(contours_blue[0]));
				if (contours_blue.size()!=0 && contourArea(contours_blue[0])>1500000)
		     	{
					ROS_INFO("Contour of blue_size being detected, area=%d", contourArea(contours_blue[0]));
