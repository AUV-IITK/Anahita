...
void imu_data_callback(sensor_msgs::Imu msg)
{
  ...
  imu_pitch_pub.publish(imu_pitch);
	...
	ROS_INFO("Sparton IMU Data (Roll, Pitch, Yaw) = (%.4f, %.4f, %.4f)", imu_roll.data, imu_pitch.data, imu_yaw.data);
}

int main(int argc, char **argv)
{
  // initiliazing ROS node
  ros::init(argc, argv, "sparton");
  ros::NodeHandle nh;
  ...
  ros::Subscriber imu_data_sub = nh.subscribe<sensor_msgs::Imu>("/sparton/imu/data", 1000, &imu_data_callback);
